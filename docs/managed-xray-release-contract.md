...
| v1 | `managed`, no release key | `config_validate`, `config_apply` | Manages configuration only. Signed Xray updates remain disabled. |
| v1 | `managed`, valid release key, no signed bundle applied | `config_validate`, `config_apply`, `xray_update` | The configured Xray path is the bootstrap fallback for the first signed bundle. |
| v1 | `managed`, valid release key, signed bundle applied | `config_validate`, `config_apply`, `xray_update` | Uses only the selected managed bundle; the bootstrap path is ignored. |
...
1. The Ed25519 signature on manifest schema 1 verifies against the Agent's pinned release key.
...
3. The bundle contains `xray`, `geoip.dat`, and `geosite.dat`; the candidate reports the manifest's separate `xrayVersion` value.
4. If a managed configuration already exists, the candidate accepts that exact configuration with `xray run -test` using the candidate geodata.

This runtime validation is the Xray configuration compatibility gate. A signed version that cannot load the current node configuration is rejected before the active runtime is stopped.
...
The manifest `version` is the unique Fork Release tag, for example `v26.7.28-xui.1`. `xrayVersion` is the value reported by the executable, for example `26.7.28`. The manifest also binds publication time, platform, archive URL, archive size, and SHA-256. Xray releases use the same pinned Ed25519 trust root as Agent releases, but use separate manifests and state directories.
...
- With an applied configuration, the candidate is validated first. The Agent then stops the current process, switches the complete runtime bundle atomically, and starts a replacement process. On the first signed-bundle installation, the current process may still use the configured bootstrap executable.
- The replacement must become stable within the managed process health deadline. Failure restores the previous signed bundle, or the bootstrap executable when no signed bundle has yet been confirmed, and restarts it using a recovery context that does not inherit control-channel cancellation.
...
2. Publish signed Xray release manifests from the maintained fork.
...
2. Request the previous signed Xray release. The same candidate validation and managed restart path applies to downgrades.
